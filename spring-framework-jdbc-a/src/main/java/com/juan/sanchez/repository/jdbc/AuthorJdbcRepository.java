...
class AuthorJdbcRepository implements AuthorRepository {
    ...
    public Author findById(int id) {
        try {
            return jdbcTemplate.queryForObject("SELECT * FROM author WHERE id=?", new AuthorRowMapper(), id);
        }
    ...
    public Author findByIdWithBooks(int id) {
        try {
            return jdbcTemplate.query("SELECT a.id as id_author, b.id as id_book, a.*, b.* " +
                            ...
                            "ON a.id=b.author_id " +
                            "WHERE a.id=?",
                    new AuthorResultSetExtractor(), id);
        }
        catch(EmptyResultDataAccessException ex) {
            return null;
        }
     ...
     */
    private class AuthorsResultSetExtractor implements ResultSetExtractor<Set<Author>> {

        @Override
        public Set<Author> extractData(ResultSet rs) throws SQLException, DataAccessException {
